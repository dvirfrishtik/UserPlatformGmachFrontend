...
function StepSelectPayment({
  ...
  const paymentModeOptions: { mode: PurchasePaymentMode; label: string }[] = [
    { mode: "bank", label: "הוראת קבע בנקאית" },
    { mode: "credit", label: "כרטיס אשראי" },
  ];
  const modeCount = paymentModeOptions.length;
  ...
  return (
    <div className="flex flex-col items-stretch w-full max-w-[640px] mx-auto">
      ...
      <div style={{ marginBottom: "32px" }}>
        ...
        >
          {paymentModeOptions.map((opt, i) => {
            const selected = paymentMode === opt.mode;
            const isFirst = i === 0;
            const isLast = i === modeCount - 1;
            return (
              <button
                key={opt.mode}
                type="button"
                ...
                onClick={() => onPaymentModeChange(opt.mode)}
        ...
        <div
          className="flex sm:hidden flex-row gap-2 overflow-x-auto pb-1"
          dir="rtl"
          role="radiogroup"
          aria-label="אמצעי תשלום"
        >
